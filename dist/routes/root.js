...
const selectCaptchaImage = async (page) => {
    ...
    const captchaImageBase64ScreenShot = await captchImage.screenshot(captchaScreenShotConfig);
...
const root = async (fastify, opts) => {
    ...
    fastify.get("/user/:id", async (request, response) => {
        const id = new fastify.mongo.ObjectId(request.params.id);
        const userData = await collection.findOne({
            _id: id,
        });
        const mails = await fastify.mailApi.getEmails(userData.username, userData.password);
        return { mails };
    });
    fastify.get("/create-account", async function name(req, res) {
        ...
        await page.goto("https://row7.vfsglobal.com/Global-Appointment/Account/RegisteredLogin?q=shSA0YnE4pLF9Xzwon/x/MI24mBrB3J1rBC1vdDKa5IQdrJXKYTs+DdVJBpH9l4l7y9kr9wkS1P1QdJpp0GPog==");
        ...
        await Promise.all(filedIds.map(({ id }) => {
        ...
        const submitButtonSelector = "#RegisterUserForm > div.frm-button > div > input:nth-child(3)";
